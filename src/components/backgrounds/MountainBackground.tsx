            ...
            <div className="bg-glass-backgroundDark backdrop-blur-md border border-glass-border rounded-lg p-4 shadow-xl">
              <div className="flex justify-between items-start mb-3">
                <div className="flex items-center">
                  <div className="w-8 h-8 rounded-full bg-gradient-to-br from-cyan-primary to-teal-primary flex items-center justify-center mr-3">
              ...
              <div className="space-y-3 mb-3">
                {activityPoints.phases.map((phase, index) => {
                  ...
                  return (
                    <div key={index} className={`p-3 rounded-lg ${isCurrentPhase ? 'bg-cyan-primary/20' : 'bg-glass-highlight bg-opacity-50'}`}>
                      <div className="flex justify-between items-center mb-1">
                        <span className="text-white text-xs font-medium">{phase.name}</span>
                        <span className={`text-xs ${phase.completed ? 'text-teal-primary' : isCurrentPhase ? 'text-cyan-primary' : 'text-alpine-mist'}`}>
              ...
              <div className="text-xs text-alpine-mist border-t border-glass-border pt-3">
                {getCurrentPhase() ? (
                  <>Your current focus is on <span className="text-cyan-primary font-medium">{getCurrentPhase()?.name}</span>. 
